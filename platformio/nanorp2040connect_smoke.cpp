...
slmp4e::Slmp4eClient client(transport, tx_buffer, sizeof(tx_buffer), rx_buffer, sizeof(rx_buffer));
...
void setup() {
    ...
    (void)WiFi.begin(kWifiSsid, kWifiPassword);

    slmp4e::TypeNameInfo info = {};
    (void)client.connect(kPlcHost, kPlcPort);
    (void)client.readTypeName(info);
}
