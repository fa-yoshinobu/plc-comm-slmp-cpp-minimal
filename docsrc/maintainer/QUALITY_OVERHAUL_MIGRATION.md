...
- M5Stack StampPLC ESP32-S3 was built from the current sibling worktree and uploaded through COM8 on 2026-07-17. With profile `melsec:iq-r`, TCP endpoint `192.168.250.100:1025`, and read-only device `D100`, the first read returned `0`, the socket reported `SO_KEEPALIVE=8` (lwIP's nonzero `SOF_KEEPALIVE` bit) and `TCP_KEEPIDLE=30`, and the same descriptor `48` returned `D100=0` after 35 seconds idle. The final result recorded two requests, 54 transmitted bytes, and 34 received bytes. The first console run incorrectly required the enabled value to equal `1`; the console predicate was corrected to accept any nonzero value and the identical live check then passed.
- The same StampPLC setup used WiFiUDP with requested local port `0` against `192.168.250.100:1035`. The matching one-word response returned `D100=0` and recorded one request, 27 transmitted bytes, and 17 received bytes. Together with the inspected `begin(0)` to `bind(0)` path, receipt on that socket proves successful ESP32 ephemeral binding even though the Arduino wrapper does not expose the assigned numeric port. Neither StampPLC check sent a PLC write.
...
- [x] Profile: `melsec:iq-r`; platform: M5Stack StampPLC ESP32-S3 WiFiClient; endpoint: `192.168.250.100:1025`; device: `D100`; intent: read only; result: the initial and post-35-second-idle reads both returned `0`, descriptor `48` remained unchanged, and socket evidence reported nonzero `SO_KEEPALIVE=8` plus `TCP_KEEPIDLE=30`. The corrected console classified the check as PASS on 2026-07-17. Restoration: none because no write was performed.
- [x] Profile: `melsec:iq-r`; platform: M5Stack StampPLC ESP32-S3 WiFiUDP; endpoint: `192.168.250.100:1035`; device: `D100`; intent: read only; result: local port `0` reached the inspected ESP32 `bind(0)` path and the same socket received the matching response with `D100=0`, proving successful ephemeral assignment. The Arduino wrapper does not expose the assigned numeric value. Restoration: none because no write was performed.
