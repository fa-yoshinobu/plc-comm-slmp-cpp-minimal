...
enum class Error : uint8_t {
    ...
    UnsupportedDevice,      ///< The requested device code is not supported.
    BufferTooSmall,         ///< Provided buffer capacity is insufficient.
    NotConnected,           ///< Transport layer is not connected.
    TransportError,         ///< Error occurred in the transport layer (e.g., TCP/UDP).
    ProtocolError,          ///< Malformed packet or protocol violation detected.
    PlcError,               ///< PLC returned a non-zero end code.
    Busy,                   ///< An asynchronous operation is already in progress.
};

/**
 * @enum FrameType
 * @brief SLMP frame formats.
 */
enum class FrameType : uint8_t {
    Frame3E,                ///< Standard 3E frame (most common for Q/L/iQ-R).
    Frame4E,                ///< 4E frame (extended functionality).
};
...
enum class CompatibilityMode : uint8_t {
    iQR,     ///< Use subcommands 0x0002/0x0003 (iQ-R series extensions).
...
/**
 * @enum DeviceCode
 * @brief SLMP binary device codes.
 */
 ...
 */
struct DeviceAddress {
    DeviceCode code;        ///< Device type code.
    uint32_t number;        ///< Numeric address (index).
};
 ...
 * 
 * Supports both synchronous (blocking) and asynchronous (non-blocking) device access.
 * Uses external buffers to minimize memory allocations.
 * 
...
class SlmpClient {
  public:
     ...
     * @param tx_capacity Transmission buffer capacity.
     * @param rx_buffer Reception buffer.
     * @param rx_capacity Reception buffer capacity.
     */
    SlmpClient(
    ...
    /** @brief Set SLMP monitoring timer (units of 250ms). */
    ...
    /** @brief Get last logical error. */
    Error lastError() const;
    /** @brief Get last PLC end code (error code from PLC). */
    uint16_t lastEndCode() const;
    
    /** @brief Access last sent request frame. */
    const uint8_t* lastRequestFrame() const;
    /** @brief Length of last sent request frame. */
    size_t lastRequestFrameLength() const;
    /** @brief Access last received response frame. */
    const uint8_t* lastResponseFrame() const;
    /** @brief Length of last received response frame. */
    size_t lastResponseFrameLength() const;

    // --- Synchronous (Blocking) API ---

    /** @brief Read PLC model and type name. */
    ...
    /** @brief Clear PLC error state. */
    Error clearError();
    /** @brief Unlock remote password. */
    Error remotePasswordUnlock(const char* password);
    /** @brief Lock remote password. */
    Error remotePasswordLock(const char* password);

    // --- Asynchronous (Non-blocking) API ---

    /** @brief Advance asynchronous state machine. Call this frequently in loop. */
    void update(uint32_t now_ms);
    /** @brief True if an asynchronous operation is in progress. */
    bool isBusy() const;

    /** @brief Start async ReadTypeName. */
    Error beginReadTypeName(TypeNameInfo& out, uint32_t now_ms);
    /** @brief Start async ReadWords. */
    ...
    /** @brief Start async WriteBlock (simple). */
    Error beginWriteBlock(
        const DeviceBlockWrite* word_blocks,
        size_t word_block_count,
        const DeviceBlockWrite* bit_blocks,
    ...
    /** @brief Start async RemotePasswordLock(const char* password, uint32_t now_ms). */
